...
from .adaptive_agent import RuleChangeApprovalManager
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--adaptive-rule-change", type=parse_bool, choices=[True, False], metavar="true|false", help="Enable or disable adaptive AI rule-change proposals")
    parser.add_argument("--adaptive-day-threshold", type=int, help="Consecutive failing days before AI proposes rule changes")
    parser.add_argument("--approve-rule-change", help="Apply a pending adaptive rule-change proposal by proposal id")
    parser.add_argument("--apply-db-schema-change", type=parse_bool, choices=[True, False], metavar="true|false", default=False, help="Apply approved database schema changes when a proposal includes SQL")
    parser.add_argument("--notification-email-to", help="Email recipient for approval/schema notifications")
...
def load_config(config_path: Path | None, args: argparse.Namespace) -> PipelineConfig:
    ...
    env_map = {
        ...
        "adaptive_rule_change_enabled": "ADAPTIVE_RULE_CHANGE_ENABLED",
        "notification_email_to": "NOTIFICATION_EMAIL_TO",
        "smtp_host": "SMTP_HOST",
    }
    for key, env_name in env_map.items():
        if os.getenv(env_name):
            data[key] = parse_bool(os.environ[env_name]) if key in {"database_load_enabled", "adaptive_rule_change_enabled"} else os.environ[env_name]
    if args.watch_dir:
    ...
    if args.adaptive_rule_change is not None:
        data["adaptive_rule_change_enabled"] = args.adaptive_rule_change
    if args.adaptive_day_threshold is not None:
        data["adaptive_day_threshold"] = args.adaptive_day_threshold
    if args.notification_email_to:
        data["notification_email_to"] = args.notification_email_to
    path_keys = [
        "watch_dir",
        "processing_dir",
        "accepted_dir",
...
def main() -> None:
    ...
    if args.approve_rule_change:
        summary = RuleChangeApprovalManager(config).approve(args.approve_rule_change, args.apply_db_schema_change)
