...
class PipelineConfig:
    ...
    adaptive_rule_change_enabled: bool = False
    adaptive_day_threshold: int = 10
    adaptive_history_file: Path = Path("data/futures/adaptive/history.json")
    rule_change_dir: Path = Path("data/futures/rule_changes")
    notification_outbox_dir: Path = Path("data/futures/notifications")
    notification_email_to: str | None = None
    notification_email_from: str = "ai-file-monitor@example.com"
    ...
    def ensure_directories(self) -> None:
        for directory in (
            ...
            self.archive_dir,
            self.reason_dir,
            self.rule_change_dir,
            self.notification_outbox_dir,
        ):
            directory.mkdir(parents=True, exist_ok=True)
        if self.dry_run_load_path:
            self.dry_run_load_path.parent.mkdir(parents=True, exist_ok=True)
        self.adaptive_history_file.parent.mkdir(parents=True, exist_ok=True)
